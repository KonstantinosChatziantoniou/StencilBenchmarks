...
__global__ void my_16_4(float* g_input, float* g_output, // float* g_vsg, = 1
                        const int dimx, const int dimy, const int dimz)
{
    ...
    int tyr = threadIdx.y + radius;
    int tidx = (threadIdx.x + 16*threadIdx.y)%32;
    int tidy = (threadIdx.x + 16*threadIdx.y)/32;
...
float* myStencil(float* data, int dimx, int dimy, int dimz){
    ...
    my_16_4<<<blocks,threads>>>(dev_data, dev_out, dimx,dimy, dimz);
